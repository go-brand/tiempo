...
describe('isPlainTimeBefore', () => {
  it('returns true when time1 is before time2', () => {
    const morning = Temporal.PlainTime.from('09:00');
    const evening = Temporal.PlainTime.from('17:00');

    expect(isPlainTimeBefore(morning, evening)).toBe(true);
  });

  it('returns false when time1 is after time2', () => {
    const morning = Temporal.PlainTime.from('09:00');
    const evening = Temporal.PlainTime.from('17:00');

    expect(isPlainTimeBefore(evening, morning)).toBe(false);
  });
  ...
  it('returns false when times are equal', () => {
    ...
    expect(isPlainTimeBefore(nineAM_a, nineAM_b)).toBe(false);
  });
  ...
  it('compares across hours', () => {
    ...
    expect(isPlainTimeBefore(endOfHour, startOfNextHour)).toBe(true);
    expect(isPlainTimeBefore(startOfNextHour, endOfHour)).toBe(false);
  });
  ...
  it('compares minutes within same hour', () => {
    ...
    expect(isPlainTimeBefore(early, late)).toBe(true);
    expect(isPlainTimeBefore(late, early)).toBe(false);
  });
  ...
  it('handles midnight', () => {
    ...
    expect(isPlainTimeBefore(midnight, morning)).toBe(true);
  });
  ...
  it('handles end of day', () => {
    ...
    expect(isPlainTimeBefore(evening, endOfDay)).toBe(true);
  });
  ...
  it('distinguishes nanosecond differences', () => {
    ...
    expect(isPlainTimeBefore(time1, time2)).toBe(true);
    expect(isPlainTimeBefore(time2, time1)).toBe(false);
  });
});
